...
<div class="container">
			...
			<table class="table_text_align_center memberInfo_table">
				<tbody>
					<tr>
						...
						<td style="width: *">${memberInfo.user_name} | ${memberInfo.email}</td>
					</tr>
				</tbody>
			</table>
		</div>
					...
					<table>
						...
						<tbody>
							...
							<tr>
								<th>주소</th>
								<td>
									${memberInfo.address1}<br>${memberInfo.address2}	
									<!-- '기존 배송지 정보 ', 혹은 '직접 입력 배송지 정소'를 판별하기 위한 T/F 폼 입력 추가-->									
									<input class="selectAddress" value="T" type="hidden">	
									<input class="addressee_input" value="${memberInfo.user_name}" type="hidden">
									<input class="address1_input" type="hidden" value="${memberInfo.address1}">	
	...
	<div class="orderGoods_div">
		<!-- 상품 종류 -->
		<div class="goods_div">
			주문상품 <span class="goods_div_count"></span>개
		</div>
		<!-- 상품 테이블 -->
			...
			<tbody>
				...
				<div class="row"></div>	
					<tr>					
						<td>
						<!-- 이미지 <td>-->
						<img src="${ol.image}" width=60px height="60px" align="left" />							
						</td>
						...
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							...
							<input type="hidden" class="individual_count_input" value="${ol.product_count}">
							<input type="hidden" class="individual_totalprice_input" value="${ol.price * ol.product_count}">
							<input type="hidden" class="individual_id_input" value="${ol.product_id}">
						</td>
					</tr>
...
<form class="order_form" action="/shoporder" method="post">
	<!-- 주문자 id -->
	<input name="member_id" type="hidden" value="${memberInfo.id}">		
	<!-- 주소록, 받는사람 -->
	<input name="addressee" type="hidden">		
	<input name="memberaddr1" type="hidden">
	<input name="memberaddr2" type="hidden">
	<!-- 상품 정보 input 태그는 동적으로 추가  -->			
</form>
...
/* 총 주문 정보 세팅(우측 상단) - 배송비, 총 가격, 물품 수 */
...
/* 값 삽입 */	
	...
	// 총 갯수
	$(".goods_div_count").text(totalCount);
	// 배송비
...
$(".order_btn").on("click", function(){
	...
	let form_contents = '';
	$(".goods_table_price_td").each(function(index, element){
		let product_id = $(element).find(".individual_id_input").val();	
